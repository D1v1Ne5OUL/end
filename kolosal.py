...
class MainApp(ctk.CTkToplevel):
    ...
    def create_all_data_tab(self):
        ...
        main_scroll.bind("<MouseWheel>", self.on_scroll)
        main_scroll.bind("<Button-4>", self.on_scroll)
        main_scroll.bind("<Button-5>", self.on_scroll)
        main_scroll.bind("<Configure>", self.save_scroll_position)
    
    def on_scroll(self, event):
        self.save_scroll_position()
    
    def save_scroll_position(self, event=None):
        if hasattr(self, 'all_data_scroll'):
            try:
                canvas = self.all_data_scroll._parent_canvas
                if canvas:
                    self.scroll_positions['all_data'] = canvas.yview()[0]
            except:
                pass
    
    def restore_scroll_position(self):
        if hasattr(self, 'all_data_scroll') and 'all_data' in self.scroll_positions:
            try:
                canvas = self.all_data_scroll._parent_canvas
                if canvas:
                    canvas.yview_moveto(self.scroll_positions['all_data'])
            except:
                pass
